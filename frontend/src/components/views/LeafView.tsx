...
import { Save, Eye, Code, ArrowRight, Pencil } from "lucide-react";
import { ent } from "../../wailsjs/go/models";
import NodeEditor from "../smart/NodeEditor";
import MarkdownRenderer from "../atomic/MarkdownRenderer";
import NodeModal from "../smart/NodeModal"; // Import the modal
...
export default function LeafView({ node }: LeafViewProps) {
  ...
  return (
    <div className="h-full flex flex-col p-6 animate-in fade-in slide-in-from-bottom-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-[#16161e] p-1 rounded border border-[#2f334d]">
            <button
              ...
              className={clsx(
                "p-1.5 rounded",
                viewMode === "edit" ? "bg-gray-700 text-white" : "text-gray-500 hover:text-white"
              )}
            >
              <Code size={16} />
            </button>
            <button
              ...
              className={clsx(
                "p-1.5 rounded",
                viewMode === "preview" ? "bg-gray-700 text-white" : "text-gray-500 hover:text-white"
              )}
            >
              <Eye size={16} />
            </button>
      ...
      {/* Global edit modal for this node */}
      <NodeModal
